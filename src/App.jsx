...
import { Plus, Cigarette, Leaf, Coffee, Wine, Candy, Pill, Smartphone, Gamepad2, Heart, Zap, Trash2, Settings, X, Link2, TrendingDown, TrendingUp, BarChart3, Check, ChevronLeft, ChevronRight, Clock, Home, Activity, Target, Lock, Sparkles, Flame, Award, StickyNote, Calendar, Tag, Lightbulb, Bookmark, BookmarkCheck, AlertCircle, Users, MapPin, Smile } from 'lucide-react';
...
const TAG_CATEGORIES = [
  {
    ...
    Icon: Smile,
    color: '#ec4899',
    tags: ['Stress', 'Langeweile', 'Genuss', 'Routine', 'Angst', 'Müdigkeit', 'Frust', 'Glück'],
  },
  {
    id: 'context',
    label: 'Wo',
    Icon: MapPin,
    color: '#0891b2',
    tags: ['Arbeit', 'Pause', 'Zuhause', 'Unterwegs', 'Party', 'Treffen', 'Auto', 'Draußen'],
  },
  {
    id: 'trigger',
    label: 'Auslöser',
    Icon: Zap,
    color: '#d97706',
    tags: ['Nach Kaffee', 'Nach Essen', 'Konflikt', 'Aufwachen', 'Vor Schlaf', 'Mit Drink', 'Wartezeit', 'Aus Gewohnheit'],
  },
  {
    id: 'social',
    label: 'Mit wem',
    Icon: Users,
    color: '#7c3aed',
    tags: ['Allein', 'Mit Freunden', 'Mit Partner', 'Familie', 'Kollegen', 'Fremde'],
  },
];

const ALL_TAGS = TAG_CATEGORIES.flatMap(c => c.tags.map(t => ({ category: c.id, tag: t, color: c.color })));
const getTagColor = (tagName) => ALL_TAGS.find(t => t.tag === tagName)?.color || '#64748b';
...
const generateInsights = (entries, tracker) => {
  const insights = [];
  if (!tracker || entries.length === 0) return insights;

  const sorted = [...entries].sort((a, b) => a.timestamp - b.timestamp);
  const totalDays = Math.max(1, Math.ceil((Date.now() - sorted[0].timestamp) / 86400000));
  const totalEntries = sorted.length;

  // Helper: confidence based on data amount
  const conf = (high, medium) => totalEntries >= high ? CONFIDENCE.HIGH : totalEntries >= medium ? CONFIDENCE.MEDIUM : null;

  // ============ INSIGHT 1: Wochentag-Anomalie ============
  if (totalEntries >= 14 && totalDays >= 14) {
    const dayCounts = [0, 0, 0, 0, 0, 0, 0];
    const dayWeights = [0, 0, 0, 0, 0, 0, 0]; // wie oft kam dieser Wochentag vor
    for (let i = 0; i < totalDays; i++) {
      const d = new Date(sorted[0].timestamp + i * 86400000);
      dayWeights[d.getDay()]++;
    }
    sorted.forEach(e => dayCounts[new Date(e.timestamp).getDay()]++);
    const dayAvgs = dayCounts.map((c, i) => dayWeights[i] > 0 ? c / dayWeights[i] : 0);
    const overall = dayAvgs.reduce((a, b) => a + b, 0) / 7;
    let maxDay = 0;
    let maxDeviation = 0;
    dayAvgs.forEach((avg, i) => {
      const dev = overall > 0 ? (avg - overall) / overall : 0;
      if (dev > maxDeviation) { maxDeviation = dev; maxDay = i; }
    });
    if (maxDeviation > 0.25) {
      ...
      insights.push({
        id: `weekday_${maxDay}`,
        type: 'weekday',
        confidence: conf(28, 14),
        title: `${dayNames[maxDay]}s ist dein schwierigster Tag`,
        body: `An ${dayNames[maxDay]}en hast du im Schnitt ${Math.round(maxDeviation * 100)}% mehr Einträge als an anderen Tagen.`,
        basis: `Basis: ${dayWeights[maxDay]} ${dayNames[maxDay]}e, ${totalDays} Tage Daten`,
        action: `Was ist ${dayNames[maxDay]}s anders?`,
        Icon: Calendar,
      });
    }
  }

  // ============ INSIGHT 2: Tageszeit-Hotspot ============
  if (totalEntries >= 20) {
    // 2h Fenster
    ...
    let maxRatio = 0;
    buckets.forEach((c, i) => {
      const ratio = c / totalEntries;
      if (ratio > maxRatio) { maxRatio = ratio; maxBucket = i; }
    });
    if (maxRatio > 0.18) {
      const fromH = maxBucket * 2;
      const toH = fromH + 2;
      insights.push({
        id: `hotspot_${maxBucket}`,
        type: 'hotspot',
        confidence: conf(50, 20),
        title: `Dein kritischstes Fenster: ${fromH}-${toH} Uhr`,
        body: `${Math.round(maxRatio * 100)}% deiner Einträge fallen in dieses Zeitfenster.`,
        basis: `Basis: ${totalEntries} Einträge, ${totalDays} Tage`,
        action: `Erinnerung um ${(fromH - 1).toString().padStart(2, '0')}:30 setzen`,
        Icon: Clock,
      });
    }
  }

  // ============ INSIGHT 3: Top-Tag (Trigger) ============
  const taggedEntries = sorted.filter(e => e.tags && e.tags.length > 0);
  if (taggedEntries.length >= 10) {
    const tagCounts = {};
    taggedEntries.forEach(e => e.tags.forEach(t => { tagCounts[t] = (tagCounts[t] || 0) + 1; }));
    const sortedTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]);
    const [topTag, topCount] = sortedTags[0];
    const topRatio = topCount / taggedEntries.length;
    if (topRatio > 0.25) {
      insights.push({
        id: `toptag_${topTag}`,
        type: 'tag',
        confidence: taggedEntries.length >= 20 ? CONFIDENCE.HIGH : CONFIDENCE.MEDIUM,
        title: `"${topTag}" ist dein häufigster Trigger`,
        body: `${Math.round(topRatio * 100)}% deiner getaggten Einträge enthalten "${topTag}".`,
        basis: `Basis: ${taggedEntries.length} getaggte Einträge`,
        action: `Was hilft dir bei ${topTag}?`,
        Icon: Tag,
        tagColor: getTagColor(topTag),
      });
    }
  }

  // ============ INSIGHT 4: Tag-Folgewirkung ============
  if (taggedEntries.length >= 15) {
    // Welcher Tag führt zu kürzeren Pausen danach?
    const tagFollowGaps = {};
    for (let i = 0; i < sorted.length - 1; i++) {
      const e = sorted[i];
      const next = sorted[i + 1];
      const gap = next.timestamp - e.timestamp;
      if (gap > 4 * 3600000) continue; // ignore gaps > 4h
      if (e.tags && e.tags.length > 0) {
        e.tags.forEach(t => {
          if (!tagFollowGaps[t]) tagFollowGaps[t] = [];
          tagFollowGaps[t].push(gap);
        });
      }
    }
    const overallAvg = getAverageGap(sorted) || 0;
    let worstTag = null;
    let worstAvg = overallAvg;
    Object.entries(tagFollowGaps).forEach(([tag, gaps]) => {
      if (gaps.length < 5) return;
      const avg = gaps.reduce((a, b) => a + b, 0) / gaps.length;
      if (avg < worstAvg * 0.7) { worstTag = tag; worstAvg = avg; }
    });
    if (worstTag) {
      insights.push({
        id: `followgap_${worstTag}`,
        type: 'tag_effect',
        confidence: tagFollowGaps[worstTag].length >= 10 ? CONFIDENCE.HIGH : CONFIDENCE.MEDIUM,
        title: `"${worstTag}" verkürzt deine Pausen`,
        body: `Nach Einträgen mit "${worstTag}" folgt der nächste schon nach ${formatMinutes(worstAvg)} (statt ${formatMinutes(overallAvg)}).`,
        basis: `Basis: ${tagFollowGaps[worstTag].length} Folge-Einträge`,
        action: 'Diese Trigger gezielt vermeiden',
        Icon: Zap,
        tagColor: getTagColor(worstTag),
      });
    }
  }

  // ============ INSIGHT 5: Cluster-Verhalten ============
  if (totalEntries >= 30) {
    let longBreakClusters = 0;
    let longBreakTotal = 0;
    for (let i = 1; i < sorted.length - 1; i++) {
      const prevGap = sorted[i].timestamp - sorted[i - 1].timestamp;
      const nextGap = sorted[i + 1].timestamp - sorted[i].timestamp;
      if (prevGap > 3 * 3600000) {
        longBreakTotal++;
        if (nextGap < 30 * 60000) longBreakClusters++;
      }
    }
    if (longBreakTotal >= 5) {
      const clusterRatio = longBreakClusters / longBreakTotal;
      if (clusterRatio > 0.4) {
        insights.push({
          id: 'cluster',
          type: 'cluster',
          confidence: longBreakTotal >= 10 ? CONFIDENCE.HIGH : CONFIDENCE.MEDIUM,
          title: 'Du fällst nach langen Pausen schnell zurück',
          body: `Wenn du nach 3+ Stunden Pause +1 machst, folgt zu ${Math.round(clusterRatio * 100)}% innerhalb von 30 Minuten ein zweites.`,
          basis: `Basis: ${longBreakTotal} lange Pausen analysiert`,
          action: 'Nach Pausen bewusst durchhalten',
          Icon: AlertCircle,
        });
      }
    }
  ...
  // ============ INSIGHT 6: Wochenend vs Werktag ============
  if (totalEntries >= 20 && totalDays >= 14) {
    let weCount = 0, wtCount = 0, weDays = 0, wtDays = 0;
    for (let i = 0; i < totalDays; i++) {
      const d = new Date(sorted[0].timestamp + i * 86400000);
      const isWE = d.getDay() === 0 || d.getDay() === 6;
      if (isWE) weDays++; else wtDays++;
    }
    sorted.forEach(e => {
      const d = new Date(e.timestamp).getDay();
      if (d === 0 || d === 6) weCount++; else wtCount++;
    });
    const weAvg = weDays > 0 ? weCount / weDays : 0;
    const wtAvg = wtDays > 0 ? wtCount / wtDays : 0;
    if (wtAvg > 0 && Math.abs(weAvg - wtAvg) / wtAvg > 0.3) {
      const more = weAvg > wtAvg ? 'Wochenende' : 'Werktagen';
      const less = weAvg > wtAvg ? 'Werktagen' : 'Wochenende';
      const diff = Math.round((Math.abs(weAvg - wtAvg) / Math.min(weAvg, wtAvg)) * 100);
      insights.push({
        id: 'we_vs_wt',
        type: 'weekend',
        confidence: weDays >= 4 ? CONFIDENCE.HIGH : CONFIDENCE.MEDIUM,
        title: `Am ${more} ${diff}% mehr als an ${less}`,
        body: `Ø ${weAvg.toFixed(1)} pro Tag am Wochenende vs ${wtAvg.toFixed(1)} pro Tag werktags.`,
        basis: `Basis: ${weDays} Wochenend-Tage, ${wtDays} Werktage`,
        action: 'Wochenend-Routine überdenken',
        Icon: Calendar,
      });
    }
  }

  // ============ INSIGHT 7: Erstes +1 des Tages ============
  if (totalEntries >= 20 && totalDays >= 7) {
    const dayFirsts = {};
    sorted.forEach(e => {
      const d = new Date(e.timestamp);
      const key = d.toDateString();
      if (!dayFirsts[key] || e.timestamp < dayFirsts[key].timestamp) dayFirsts[key] = e;
    });
    const firstHours = Object.values(dayFirsts).map(e => new Date(e.timestamp).getHours() + new Date(e.timestamp).getMinutes() / 60);
    const avgFirst = firstHours.reduce((a, b) => a + b, 0) / firstHours.length;
    // Compare days where first was early vs late
    const earlyDays = Object.entries(dayFirsts).filter(([_, e]) => new Date(e.timestamp).getHours() + new Date(e.timestamp).getMinutes() / 60 < avgFirst);
    const lateDays = Object.entries(dayFirsts).filter(([_, e]) => new Date(e.timestamp).getHours() + new Date(e.timestamp).getMinutes() / 60 >= avgFirst);
    if (earlyDays.length >= 3 && lateDays.length >= 3) {
      const earlyTotal = earlyDays.reduce((sum, [key]) => sum + sorted.filter(s => new Date(s.timestamp).toDateString() === key).length, 0);
      const lateTotal = lateDays.reduce((sum, [key]) => sum + sorted.filter(s => new Date(s.timestamp).toDateString() === key).length, 0);
      const earlyAvg = earlyTotal / earlyDays.length;
      const lateAvg = lateTotal / lateDays.length;
      if (earlyAvg > lateAvg * 1.25) {
        const diff = Math.round(((earlyAvg - lateAvg) / lateAvg) * 100);
        const avgH = Math.floor(avgFirst);
        const avgM = Math.round((avgFirst - avgH) * 60);
        insights.push({
          id: 'early_start',
          type: 'early_start',
          confidence: (earlyDays.length + lateDays.length) >= 14 ? CONFIDENCE.HIGH : CONFIDENCE.MEDIUM,
          title: 'Früher Start = schwierigerer Tag',
          body: `An Tagen wo du vor ${avgH}:${avgM.toString().padStart(2, '0')} startest, hast du ${diff}% mehr +1.`,
          basis: `Basis: ${earlyDays.length} frühe + ${lateDays.length} späte Tage`,
          action: 'Morgens länger durchhalten',
          Icon: TrendingUp,
        });
      }
    }
...
const DEMO_INSIGHTS = [
  {
    ...
    title: 'Mittwochs ist dein schwierigster Tag',
    body: 'An Mittwochen hast du im Schnitt 35% mehr Einträge als an anderen Tagen.',
    basis: 'Beispiel — verfügbar ab 2 Wochen Daten',
    action: 'Was ist mittwochs anders?',
    Icon: Calendar,
  },
  {
    id: 'demo_hotspot',
    type: 'hotspot',
    confidence: CONFIDENCE.DEMO,
    title: 'Dein kritischstes Fenster: 14-16 Uhr',
    body: '28% deiner Einträge fallen in dieses Zeitfenster.',
    basis: 'Beispiel — verfügbar ab 20 Einträgen',
    action: 'Erinnerung um 13:30 setzen',
    Icon: Clock,
  },
  {
    id: 'demo_tag',
    type: 'tag',
    confidence: CONFIDENCE.DEMO,
    title: '"Stress" ist dein häufigster Trigger',
    body: '47% deiner getaggten Einträge enthalten "Stress".',
    basis: 'Beispiel — verfügbar ab 10 getaggten Einträgen',
    action: 'Was hilft dir bei Stress?',
    Icon: Tag,
    tagColor: '#ec4899',
  },
  {
    ...
    confidence: CONFIDENCE.DEMO,
    title: 'Du fällst nach langen Pausen schnell zurück',
    body: 'Wenn du nach 3+ Stunden Pause +1 machst, folgt zu 70% innerhalb von 30 Minuten ein zweites.',
    basis: 'Beispiel — verfügbar ab 30 Einträgen',
    action: 'Nach Pausen bewusst durchhalten',
    Icon: AlertCircle,
  },
];
...
export default function App() {
  ...
  const [savedInsights, setSavedInsights] = useState([]); // [{insightId, savedAt}]
  const [dismissedInsights, setDismissedInsights] = useState([]); // [insightId]
  const [seenInsightIds, setSeenInsightIds] = useState([]); // tracks "already seen" for new-badge
  const [loading, setLoading] = useState(true);
  const [now, setNow] = useState(Date.now());
  const [tab, setTab] = useState('home');
  const [modal, setModal] = useState(null); // {type, data}
  const [snackbar, setSnackbar] = useState(null); // {entryId, trackerId, color}
  ...
  useEffect(() => {
    try {
      ...
      const si = localStorage.getItem('pausa_savedInsights');
      const di = localStorage.getItem('pausa_dismissedInsights');
      const seen = localStorage.getItem('pausa_seenInsights');
      if (t) setTrackers(JSON.parse(t));
      if (e) setEntries(JSON.parse(e));
      if (c) setCombos(JSON.parse(c));
      if (p) setIsPremium(JSON.parse(p));
      if (si) setSavedInsights(JSON.parse(si));
      if (di) setDismissedInsights(JSON.parse(di));
      if (seen) setSeenInsightIds(JSON.parse(seen));
  ...
  const persistSavedInsights = (next) => { setSavedInsights(next); save('savedInsights', next); };
  const persistDismissedInsights = (next) => { setDismissedInsights(next); save('dismissedInsights', next); };
  const persistSeenInsights = (next) => { setSeenInsightIds(next); save('seenInsights', next); };
  ...
  const logEntry = (trackerId, offsetMinutes = 0, note = '', tags = []) => {
    const entry = {
    ...
    // Snackbar nur wenn ohne Tags eingetragen (sonst hat User Tags schon gewählt)
    if (tags.length === 0) {
      const tracker = trackers.find(t => t.id === trackerId);
      setSnackbar({ entryId: entry.id, trackerId, color: tracker?.color || '#0f172a' });
    }
    return entry;
  };

  const addTagsToEntry = (entryId, tags) => {
    persistEntries(entries.map(e => e.id === entryId ? { ...e, tags: [...(e.tags || []), ...tags] } : e));
  };
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; -webkit-user-select: none; user-select: none; }
        ...
        @keyframes success-pop { 0% { transform: scale(1); } 50% { transform: scale(1.08); } 100% { transform: scale(1); } }
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: '500px', margin: '0 auto' }}>
          ...
          {tab === 'insights' && (
            <InsightsTab
              ...
              onSaveInsight={(id) => {
                if (!savedInsights.find(s => s.insightId === id)) {
                  persistSavedInsights([...savedInsights, { insightId: id, savedAt: Date.now() }]);
                }
              }}
              onUnsaveInsight={(id) => persistSavedInsights(savedInsights.filter(s => s.insightId !== id))}
              onDismissInsight={(id) => persistDismissedInsights([...dismissedInsights, id])}
              onMarkSeen={(ids) => {
                const newSeen = [...new Set([...seenInsightIds, ...ids])];
                if (newSeen.length !== seenInsightIds.length) persistSeenInsights(newSeen);
              }}
              onShowPaywall={(f) => setModal({ type: 'paywall', data: { feature: f } })}
            />
          )}
        ...
        {/* Bottom tab bar */}
        <TabBar tab={tab} setTab={setTab} insightBadge={(() => {
          // Count new (unseen) real insights across all trackers
          if (!isPremium) return 0;
          let total = 0;
          trackers.forEach(t => {
            const tEntries = entries.filter(e => e.trackerId === t.id);
            const ins = generateInsights(tEntries, t);
            ins.forEach(i => {
              if (!seenInsightIds.includes(i.id) && !dismissedInsights.includes(i.id)) total++;
            });
          });
          return total;
        })()} />
        ...
        {/* Tagging Snackbar - erscheint nach +1 */}
        {snackbar && (
          <TagSnackbar
            color={snackbar.color}
            onPick={(tags) => { addTagsToEntry(snackbar.entryId, tags); setSnackbar(null); }}
            onExpand={() => { setModal({ type: 'fullTag', data: { entryId: snackbar.entryId, color: snackbar.color } }); setSnackbar(null); }}
        ...
        {modal?.type === 'fullTag' && (
          <FullTagModal
            ...
            onClose={() => setModal(null)}
            onSave={(tags) => { addTagsToEntry(modal.data.entryId, tags); setModal(null); }}
          />
        )}
      </div>
    </>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {entry.tags && entry.tags.length > 0 && (
          <div style={{ display: 'flex', gap: '4px', marginTop: '4px', flexWrap: 'wrap' }}>
            ...
            {entry.tags.length > 3 && (
              <span style={{ fontSize: '10px', color: '#94a3b8' }}>+{entry.tags.length - 3}</span>
            )}
          </div>
        )}
        {entry.note && (
          <div style={{ fontSize: '11px', color: '#64748b', marginTop: '3px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', fontStyle: 'italic' }}>
            „{entry.note}"
          </div>
        )}
      </div>
...
function InsightsTab({ trackers, combos, entries, isPremium, now, savedInsights, dismissedInsights, seenInsightIds, onUpdateTracker, onSaveInsight, onUnsaveInsight, onDismissInsight, onMarkSeen, onShowPaywall }) {
  const [selectedId, setSelectedId] = useState(trackers[0]?.id);
  const [view, setView] = useState('new'); // 'new' | 'all' | 'saved'
  ...
  const allInsights = React.useMemo(() => generateInsights(realEntries, tracker), [realEntries, tracker]);

  // Free Tier: nur 1-2 Insight-Typen
  const FREE_INSIGHT_TYPES = ['hotspot', 'weekday'];
  const visibleRealInsights = isPremium ? allInsights : allInsights.filter(i => FREE_INSIGHT_TYPES.includes(i.type));

  // Demo insights immer sichtbar
  const demoInsights = isPremium
    ? [] // Premium-User brauchen keine Demos
    : DEMO_INSIGHTS.filter(d => !FREE_INSIGHT_TYPES.includes(d.type)); // Nur Demos die premium-only sind

  const activeReal = visibleRealInsights.filter(i => !dismissedInsights.includes(i.id));
  const newOnes = activeReal.filter(i => !seenInsightIds.includes(i.id));
  const savedReal = activeReal.filter(i => savedInsights.find(s => s.insightId === i.id));

  // Mark as seen wenn user hier ist
  React.useEffect(() => {
    if (newOnes.length > 0) {
      const timer = setTimeout(() => onMarkSeen(newOnes.map(i => i.id)), 1500);
  ...
  let displayList = [];
  if (view === 'new') {
    displayList = newOnes.length > 0 ? newOnes : activeReal;
  } else if (view === 'saved') {
    displayList = savedReal;
  } else {
    displayList = activeReal;
  }
  ...
  return (
    <>
      <header style={{ padding: '28px 20px 16px' }}>
        <div style={{ fontSize: '10px', letterSpacing: '0.25em', color: '#64748b', textTransform: 'uppercase', fontWeight: 600, marginBottom: '4px' }}>Einsichten</div>
        <h1 style={{ fontFamily: '"DM Serif Display", Georgia, serif', fontStyle: 'italic', fontSize: '32px', fontWeight: 400, margin: 0, color: '#0f172a', letterSpacing: '-0.02em' }}>
          {activeReal.length > 0 ? `${activeReal.length} Erkenntniss${activeReal.length === 1 ? '' : 'e'}` : 'Muster erkennen'}
        </h1>
        {newOnes.length > 0 && (
          <div style={{ marginTop: '8px', display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '4px 10px', background: '#dcfce7', borderRadius: '100px', fontSize: '11px', color: '#15803d', fontWeight: 600 }}>
            <Sparkles size={11} /> {newOnes.length} neu
          </div>
        )}
      </header>
      ...
      {activeReal.length > 0 && (
        <div style={{ padding: '0 16px 16px' }}>
          <div style={{ display: 'flex', gap: '4px', background: '#e2e8f0', borderRadius: '12px', padding: '3px' }}>
            <SegmentBtn active={view === 'new'} onClick={() => setView('new')}>
              Neu {newOnes.length > 0 && <span style={{ fontSize: '9px', background: '#15803d', color: 'white', padding: '1px 6px', borderRadius: '100px', marginLeft: '4px' }}>{newOnes.length}</span>}
            </SegmentBtn>
            <SegmentBtn active={view === 'all'} onClick={() => setView('all')}>Alle</SegmentBtn>
            <SegmentBtn active={view === 'saved'} onClick={() => setView('saved')}>
              Gespeichert {savedReal.length > 0 && <span style={{ fontSize: '9px', color: '#64748b', marginLeft: '4px' }}>{savedReal.length}</span>}
            </SegmentBtn>
          </div>
        </div>
      )}
      ...
      <div style={{ padding: '0 16px' }}>
        {/* Goal & Streak immer oben */}
        <GoalStreakSection tracker={tracker} entries={realEntries} onUpdateTracker={onUpdateTracker} />

        {/* Real insights */}
        {displayList.length === 0 && view !== 'saved' && (
          <NoInsightsYet entryCount={realEntries.length} isPremium={isPremium} />
        )}
        {displayList.length === 0 && view === 'saved' && (
          <div style={{ padding: '30px 20px', textAlign: 'center', color: '#94a3b8', fontSize: '13px' }}>
            Noch keine gespeicherten Einsichten. Speichere wichtige Erkenntnisse mit dem 🔖 Symbol.
          </div>
        )}
        {displayList.map(insight => (
          <InsightCardV2
            key={insight.id}
            insight={insight}
            isSaved={!!savedInsights.find(s => s.insightId === insight.id)}
            onSave={() => onSaveInsight(insight.id)}
            onUnsave={() => onUnsaveInsight(insight.id)}
            onDismiss={() => onDismissInsight(insight.id)}
          />
        ))}

        {/* Demo Insights für Free-User die anzeigen was Premium kann */}
        {!isPremium && demoInsights.length > 0 && view !== 'saved' && (
          <>
            <div style={{ marginTop: '24px', marginBottom: '8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 4px' }}>
              <div style={{ fontSize: '10px', letterSpacing: '0.25em', color: '#94a3b8', textTransform: 'uppercase', fontWeight: 600 }}>
                Mit Pro freischalten
              </div>
              <button onClick={() => onShowPaywall('insights')} style={{ background: 'transparent', border: 'none', color: '#b45309', fontSize: '11px', fontWeight: 600, cursor: 'pointer' }}>
                Mehr →
              </button>
            </div>
            {demoInsights.map(insight => (
              <InsightCardV2
                ...
                insight={insight}
                isSaved={false}
                onSave={() => onShowPaywall('insights')}
                onDismiss={() => onShowPaywall('insights')}
                onUpgrade={() => onShowPaywall('insights')}
              />
            ))}
          </>
        )}
...
function NoInsightsYet({ entryCount, isPremium }) {
  return (
    <div style={{
      background: 'white', borderRadius: '20px', padding: '24px 20px',
      textAlign: 'center', boxShadow: '0 1px 3px rgba(15, 23, 42, 0.03)',
      marginBottom: '12px',
    }}>
      <Lightbulb size={26} color="#cbd5e1" style={{ marginBottom: '10px' }} strokeWidth={1.5} />
      <div style={{ fontFamily: '"DM Serif Display", serif', fontStyle: 'italic', fontSize: '18px', color: '#475569', marginBottom: '6px' }}>
        Noch zu früh für Erkenntnisse
      </div>
      <div style={{ fontSize: '12px', color: '#64748b', lineHeight: 1.5, maxWidth: '280px', margin: '0 auto' }}>
        {entryCount < 14 ? (
          <>Erste Muster erscheinen ab etwa 14 Einträgen. Aktuell: {entryCount}. Weiter so.</>
        ) : (
          <>Deine Daten zeigen noch keine signifikanten Muster. Das ist gut – es bedeutet, dein Verhalten ist gleichmäßig.</>
        )}
      </div>
    </div>
  );
...
function InsightCardV2({ insight, isSaved, onSave, onUnsave, onDismiss, onUpgrade }) {
  const Icon = insight.Icon || Lightbulb;
  const isDemo = insight.confidence === CONFIDENCE.DEMO;
  const confColors = {
    high: { bg: '#dcfce7', text: '#15803d', label: 'Hohe Konfidenz' },
    medium: { bg: '#fef3c7', text: '#b45309', label: 'Erste Hinweise' },
    demo: { bg: '#e0e7ff', text: '#4338ca', label: 'Beispiel' },
  };
  const c = confColors[insight.confidence] || confColors.medium;
  const accent = insight.tagColor || '#0f172a';
  ...
  return (
    <div style={{
      ...
      {/* Header: Confidence + Save */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
        <div style={{
          display: 'inline-flex', alignItems: 'center', gap: '5px',
          padding: '3px 9px', background: c.bg, borderRadius: '100px',
          fontSize: '10px', color: c.text, fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase',
        }}>
          {isDemo ? <Sparkles size={10} /> : <Check size={10} />} {c.label}
        </div>
        {!isDemo && (
          <button onClick={isSaved ? onUnsave : onSave} style={{
            background: 'transparent', border: 'none', cursor: 'pointer',
            color: isSaved ? '#0f172a' : '#cbd5e1', padding: '4px',
          }}>
            {isSaved ? <BookmarkCheck size={18} fill="#0f172a" /> : <Bookmark size={18} />}
          </button>
        )}
      </div>
      ...
      {/* Basis */}
      <div style={{ fontSize: '10px', color: '#94a3b8', marginBottom: '12px', fontStyle: 'italic' }}>
        {insight.basis}
      </div>
      ...
      {insight.action && (
        <div style={{
          padding: '10px 12px',
          background: isDemo ? '#f1f5f9' : `${accent}08`,
          borderRadius: '10px',
          fontSize: '12px',
          color: isDemo ? '#64748b' : accent,
          fontWeight: 500,
          display: 'flex', alignItems: 'center', gap: '6px',
        }}>
          <Lightbulb size={12} /> {insight.action}
        </div>
      )}
      ...
      {isDemo && onUpgrade && (
        <button onClick={onUpgrade} style={{
          ...
          <Sparkles size={12} /> Mit Pro freischalten
        </button>
      )}

      {/* Dismiss for active insights */}
      {!isDemo && (
        <button onClick={onDismiss} style={{
          position: 'absolute', top: '12px', right: '46px',
          background: 'transparent', border: 'none', cursor: 'pointer',
          color: '#cbd5e1', padding: '4px', fontSize: '11px',
...
function TabBar({ tab, setTab, insightBadge = 0 }) {
  const tabs = [
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        padding: '10px 20px calc(env(safe-area-inset-bottom, 0px) + 40px)',
...
function TagSnackbar({ color, onPick, onExpand, onDismiss }) {
  const [picked, setPicked] = useState([]);
  const [autoClose, setAutoClose] = useState(true);

  // Top quick tags from feeling category
  const quickTags = ['Stress', 'Genuss', 'Routine', 'Langeweile'];

  // Auto-dismiss nach 6 Sekunden wenn nicht interagiert
  useEffect(() => {
    if (!autoClose) return;
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
        <div style={{ fontSize: '12px', color: '#0f172a', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '6px' }}>
          <Check size={13} color={color} strokeWidth={2.5} /> Eingetragen — wie war's?
        </div>
        <button onClick={onDismiss} style={{ background: 'transparent', border: 'none', color: '#94a3b8', cursor: 'pointer', padding: '2px' }}>
          <X size={14} />
        </button>
      </div>
      <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap', marginBottom: '8px' }}>
        {quickTags.map(t => (
          <button key={t} onClick={() => toggle(t)} style={{
            padding: '6px 11px',
            background: picked.includes(t) ? color : `${color}15`,
            color: picked.includes(t) ? 'white' : color,
            border: 'none',
            borderRadius: '100px',
      ...
      {picked.length > 0 && (
        <button onClick={() => onPick(picked)} style={{
          ...
          {picked.length} Tag{picked.length === 1 ? '' : 's'} speichern
        </button>
      )}
    </div>
  );
...
function FullTagModal({ color, onClose, onSave }) {
  const [picked, setPicked] = useState([]);
  ...
  return (
    <ModalShell onClose={onClose} title="Tags hinzufügen" subtitle="Was war der Kontext?">
      {TAG_CATEGORIES.map(cat => {
        ...
        return (
          <div key={cat.id} style={{ marginBottom: '18px' }}>
            <div style={{
              display: 'flex', alignItems: 'center', gap: '6px',
              fontSize: '11px', color: cat.color, textTransform: 'uppercase',
              letterSpacing: '0.15em', fontWeight: 700, marginBottom: '8px',
            }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
              {cat.tags.map(t => (
                <button key={t} onClick={() => toggle(t)} style={{
                  padding: '7px 12px',
                  background: picked.includes(t) ? cat.color : `${cat.color}12`,
                  color: picked.includes(t) ? 'white' : cat.color,
                  border: 'none',
                  borderRadius: '100px',
                  fontSize: '12px', fontWeight: 500,
                  cursor: 'pointer',
      ...
      <div style={{ display: 'flex', gap: '6px', marginTop: '8px' }}>
        ...
        <button onClick={() => onSave(picked)} disabled={picked.length === 0} style={{
          flex: 2, padding: '14px',
          background: picked.length > 0 ? '#0f172a' : '#e2e8f0',
          color: picked.length > 0 ? 'white' : '#94a3b8',
          border: 'none', borderRadius: '14px',
          cursor: picked.length > 0 ? 'pointer' : 'not-allowed',
          fontSize: '13px', fontWeight: 600,
          display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px',
        }}>
          <Check size={14} strokeWidth={2.5} /> {picked.length > 0 ? `${picked.length} Tags speichern` : 'Tags wählen'}
        </button>
      </div>
    </ModalShell>
  );
